...
|---|---|---|
| Pending | fill `#e24b4a` (red) | the action's white glyph (cross/up/pins/…) |
| Done | fill `teal/accent` `#1d9e75` | white check (two capsule bars, ±45°) |
...
| Task rows | `steps[i].actions[]` (`title`, `subtitle`, `icon`, `value`) | demo 2 |
...
`icon` keywords (`cross, up, pins, usb, lever, board, magnify, chip, recycle, label`)
map to sprites in Unity. Task done-state is **pure UI state** — nothing persists
to the backend (the completion report at 09 §7 is unchanged).
...
*Last updated: 2026-07-10 · Status: v3 approved & Editor-tested · Prev: 03 intro · Next: 05 Step 2 (content only) · Next work block: live exploded canvas*
