...
namespace DPP.UI
{
    ...
    ///   0. v3 head-follow — the viewfinder follows the full gaze ray (pitch + yaw
    ///      + roll) instead of a flattened horizontal vector. See LateUpdate.
    ...
    public class QRScanController : MonoBehaviour
    {
        ...
        [Tooltip("The scan panel lazily follows the gaze — PITCH as well as yaw — so the brackets land on whatever the participant is looking at. Smoothed so it drifts rather than sticks; a hard head-lock is uncomfortable.")]
        [SerializeField] private bool followHead = true;
        [Tooltip("Metres in front of the head along the gaze. 0.60 = arm's length, the distance the QR decode was validated at (spec 11 stage 2, ~61 ms).")]
        [SerializeField] private float followDistance = 0.60f;
        [Tooltip("Metres along the HEAD's up axis (not world up). +0.012 cancels the 12 px the bracket group sits below the panel centre, putting the frame ON the gaze point.")]
        [SerializeField] private float followHeightOffset = 0.012f;
        [Tooltip("Positional catch-up speed, /s. Lower = floatier. Raised 4 -> 6 for v3: vertical head motion is faster and larger than yaw, and at 4 the frame visibly trailed the gaze.")]
        ...
        private void LateUpdate()
        {
            ...
            // v3 (2026-07-30): follow the REAL gaze, not its horizontal projection.
            //
            // The old line here was `fwd.y = 0f`, which pinned the viewfinder to a
            // ring at eye height. Looking down at the unit on the table left the
            // brackets floating above it, and participants reasonably concluded they
            // had to lift the VCU into the frame. ZXing always decoded the WHOLE
            // camera frame — the brackets were only ever a framing aid, and the aid
            // was lying about where the camera was pointed.
            Vector3 fwd = head.transform.forward;
            if (fwd.sqrMagnitude < 1e-6f) return;
            fwd.Normalize();

            // Offset along the HEAD's up, not world up, so the correction stays
            // constant relative to the frame however far the head is pitched.
            Vector3 target = head.transform.position
                           + fwd * followDistance
                           + head.transform.up * followHeightOffset;

            float t = 1f - Mathf.Exp(-followLerp * Time.deltaTime);   // framerate-independent lazy follow
            transform.position = Vector3.Lerp(transform.position, target, t);

            // Roll follows the head (Thiago, 2026-07-30): the frame represents what
            // the headset camera sees, and that camera rolls with the head. Using
            // head.transform.up as the up-hint also keeps LookRotation well-defined
            // when looking straight down, where a world-up reference degenerates.
            Quaternion look = Quaternion.LookRotation(fwd, head.transform.up);
